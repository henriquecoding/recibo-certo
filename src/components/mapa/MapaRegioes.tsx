...
//   · Notários — atos notariais com preços (tabela nacional + honorários)
//   · Advogados — consulta e honorários por região (estimativas de mercado)
//   · Benefícios fiscais — onde vale a pena instalar a atividade (base legal)
//
// O filtro inicial vem do CONTEXTO onde o mapa está inserido (prop `contexto`),
// mas o utilizador pode alternar livremente entre camadas. Mantém toda a
// mecânica refinada dos mapas anteriores: Leaflet nativo carregado só no
// browser, fronteiras NUTS II oficiais (Eurostat), pesquisa real (Nominatim),
// GPS, scroll/arrasto bloqueáveis, toasts e lista de regiões acessível.
...
import { REGIOES_PRECO, regiaoMaisProxima, type RegiaoPreco } from "@/lib/contabilista-regioes";
import {
  PROFISSOES,
  PROFISSOES_LISTA,
  VERIFICACOES,
  ...
  type FiltroMapa,
  type ProfissaoId,
} from "@/lib/profissionais-regioes";
import {
  REGIOES_INCENTIVO,
...
const FILTROS: FiltroDef[] = [
  {
    id: "contabilistas",
    label: "Contabilistas",
    titulo: "Quanto custa um contabilista?",
    desc: "Avença mensal média por região — estimativas de mercado.",
    Icon: Calculator,
  },
  {
    id: "notarios",
    label: "Notários",
    titulo: "Quanto custam os atos notariais?",
    desc: "Procuração, testamento, habilitação de herdeiros e escrituras — com preços.",
    Icon: FileSign,
  },
  {
    id: "advogados",
    label: "Advogados",
    titulo: "Quanto custa um advogado?",
    desc: "Consulta e honorários por região — estimativas de mercado.",
    Icon: Scale,
  },
  {
    id: "beneficios",
    label: "Benefícios fiscais",
    titulo: "Onde vale a pena instalar a atividade?",
    desc: "IRC reduzido no interior, RFAI, IVA das ilhas — com base legal.",
    Icon: Flag,
  },
];
...
function nivelCamada(filtro: FiltroMapa, regiaoId: string): number {
  if (filtro === "beneficios") {
    return REGIOES_INCENTIVO.find((r) => r.id === regiaoId)?.nivel ?? 0.5;
  }
  return nivelProfissao(filtro, regiaoId);
}

/** Texto do pin de uma região na camada ativa. */
function textoPin(filtro: FiltroMapa, r: RegiaoPreco): string {
  if (filtro === "beneficios") {
    return REGIOES_INCENTIVO.find((x) => x.id === r.id)?.selo ?? r.nome;
  }
  const b = bandaProfissao(filtro, r.id);
  return `${eur0(b.min)}–${eur0(b.max)}`;
}

/** Resumo curto de uma região na camada ativa (tooltips, toasts, lista). */
function resumoRegiao(filtro: FiltroMapa, regiaoId: string): string {
  if (filtro === "beneficios") {
    return REGIOES_INCENTIVO.find((x) => x.id === regiaoId)?.headline ?? "";
  }
  const p = PROFISSOES[filtro];
  const b = bandaProfissao(filtro, regiaoId);
  return `${p.indicador}: ${eur0(b.min)}–${eur0(b.max)}${p.unidade}`;
}
...
const zoomRegiao = (id: string) => (id === "madeira" || id === "acores" ? 9 : 8);
...
export default function MapaRegioes({
  contexto = "contabilistas",
}: {
  /** Camada inicial, predefinida pelo contexto da página. O utilizador pode mudar. */
  contexto?: FiltroMapa;
}) {
  ...
  const filtroRef = useRef<FiltroMapa>(contexto);
  filtroRef.current = filtro;

  const isMobile = typeof window !== "undefined" && window.innerWidth < 768;

  const def = filtroDef(filtro);
  const profissao = filtro === "beneficios" ? null : PROFISSOES[filtro as ProfissaoId];

  const regioesOrdenadas = useMemo(() => {
    if (filtro === "beneficios") {
      return [...REGIOES_INCENTIVO].sort((a, b) => b.nivel - a.nivel).map((r) => r.id);
    }
    return [...REGIOES_PRECO]
      .sort((a, b) => {
        const ba = bandaProfissao(filtro, a.id);
        const bb = bandaProfissao(filtro, b.id);
        return bb.min + bb.max - (ba.min + ba.max) || a.nome.localeCompare(b.nome);
      })
      .map((r) => r.id);
  }, [filtro]);

  const regiaoSel = useMemo(() => REGIOES_PRECO.find((r) => r.id === selecionada) ?? null, [selecionada]);
  const incentivoSel = useMemo(() => REGIOES_INCENTIVO.find((r) => r.id === selecionada) ?? null, [selecionada]);
  ...
  const escolherResultado = (r: ResultadoGeo) => {
    ...
    mostrarNotif(r.nome, `Região ${regiao.nome} · ${resumoRegiao(filtroRef.current, regiao.id)}`, "ok");
  };
  ...
  const localizar = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        mostrarNotif("Localização ativada", `Estás na região ${regiao.nome} · ${resumoRegiao(filtroRef.current, regiao.id)}`, "ok");
      },
      () => {
        setALocalizar(false);
  ...
  // ── Pins por camada (recriados quando o filtro muda) ───────────────────────
  const renderMarcadores = useCallback(
    (f: FiltroMapa) => {
      const L = LRef.current;
      const map = mapRef.current;
      if (!L || !map) return;
      ...
      REGIOES_PRECO.forEach((r) => {
        ...
        const m = L.marker([r.lat, r.lng], {
          icon,
          title: `${r.nome}: ${resumoRegiao(f, r.id)}`,
          riseOnHover: true,
          zIndexOffset: 1000,
        })
          .addTo(map)
          .on("click", () => {
            setSelecionada(r.id);
            voarPara(r.lat, r.lng, zoomRegiao(r.id));
          });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      renderMarcadores(filtroRef.current);
      ...
      try {
        ...
        if (!cancelado && mapRef.current) {
          ...
          const layer = L.geoJSON(
            ...
            {
              style: (f) => estiloRegiao(filtroRef.current, regiaoDeFeature(f), null),
              onEachFeature: (f, lyr) => {
                const rid = regiaoDeFeature(f);
                const reg = REGIOES_PRECO.find((x) => x.id === rid);
                if (reg) {
                  // Conteúdo dinâmico: reflete a camada ativa no momento do hover.
                  lyr.bindTooltip(() => `${reg.nome} · ${resumoRegiao(filtroRef.current, reg.id)}`, { sticky: true });
                  lyr.on("click", () => {
                    setSelecionada(reg.id);
                    voarPara(reg.lat, reg.lng, zoomRegiao(reg.id));
                  });
                }
              },
            }
  ...
  const escolherRegiao = (id: string) => {
    ...
    if (r) voarPara(r.lat, r.lng, zoomRegiao(r.id));
  };
  ...
  const bandaSel = regiaoSel && filtro !== "beneficios" ? bandaProfissao(filtro, regiaoSel.id) : null;

  return (
    <div className="space-y-5 rounded-4xl border border-stone-100 bg-white p-5 shadow-card dark:border-stone-800 dark:bg-stone-900 sm:p-6">
      {/* ── Cabeçalho + filtros ── */}
      <div className="space-y-4">
        <div className="flex items-start gap-2.5">
          <span className="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-brand-light text-brand dark:bg-brand/15">
            <def.Icon size={18} />
          </span>
          <div className="min-w-0">
            <p className="font-display text-base font-semibold leading-snug text-stone-800 dark:text-stone-100">{def.titulo}</p>
            <p className="mt-0.5 text-[11px] leading-relaxed text-stone-400">{def.desc}</p>
          </div>
        </div>
          ...
          {FILTROS.map((f) => {
            ...
            return (
              <button
                ...
                className={`flex h-9 flex-shrink-0 items-center gap-1.5 rounded-full border px-3.5 text-xs font-semibold transition-all duration-200 ${
                  ativo
                    ? "border-brand bg-brand text-white shadow-glow"
                    : "border-stone-200 bg-white text-stone-600 hover:border-brand/40 hover:text-brand-dark dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:text-brand"
                }`}
              >
                <f.Icon size={13} />
                {f.label}
      ...
      <div
        ...
        role="application"
        aria-label={`Mapa de Portugal — ${def.titulo}`}
      >
        ...
        {/* Legenda — adapta-se à camada ativa */}
        <div className="flex items-center gap-3 border-t border-stone-100 bg-white px-4 py-3 dark:border-stone-800 dark:bg-stone-900">
          ...
          ) : profissao && !profissao.variacaoRegional ? (
            <>
              <span className="text-[11px] font-semibold text-stone-500 dark:text-stone-400">{profissao.indicador}</span>
              <span className="min-w-0 flex-1 truncate text-right text-[11px] text-stone-400">
                Tabela nacional — os atos tabelados custam o mesmo em todo o país
              </span>
            </>
          ) : profissao ? (
            <>
      ...
      {/* ── Região selecionada ── */}
      {regiaoSel && profissao && bandaSel && (
        <div
          key={`${filtro}-${regiaoSel.id}`}
          className="rounded-2xl border border-brand/20 bg-gradient-to-b from-brand-light/60 to-transparent p-5 dark:from-brand/8 dark:to-transparent"
          style={{ animation: "rc-mapa-in .4s cubic-bezier(0.16,1,0.3,1) both" }}
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-base font-bold leading-snug text-stone-800 dark:text-stone-100">{regiaoSel.nome}</h3>
              ...
              <p className="mt-2 font-display text-2xl font-semibold tabular-nums text-brand-dark dark:text-brand">
                {eur0(bandaSel.min)} – {eur0(bandaSel.max)}
                <span className="ml-1 text-sm font-medium text-stone-400">
                  {profissao.unidade || ""} · {profissao.indicador.toLowerCase()}
                </span>
              </p>
              {!profissao.variacaoRegional && (
                <p className="mt-1 inline-flex items-center gap-1.5 rounded-lg bg-stone-100 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-stone-500 dark:bg-stone-800 dark:text-stone-400">
                  <Info size={11} />
                  Tabela nacional — sem variação regional
                </p>
              )}
              <p className="mt-2 text-xs leading-relaxed text-stone-500 dark:text-stone-400">{profissao.descricao}</p>
            </div>
          </div>
        </div>
      )}

      {incentivoSel && filtro === "beneficios" && (
        <div
          key={`beneficios-${incentivoSel.id}`}
          className="rounded-2xl border border-brand/20 bg-gradient-to-b from-brand-light/60 to-transparent p-5 dark:from-brand/8 dark:to-transparent"
          style={{ animation: "rc-mapa-in .4s cubic-bezier(0.16,1,0.3,1) both" }}
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-base font-bold leading-snug text-stone-800 dark:text-stone-100">{incentivoSel.nome}</h3>
              <p className="text-[11px] text-stone-500 dark:text-stone-400">{incentivoSel.referencia}</p>
              <p className="mt-1 text-sm font-semibold text-brand-dark dark:text-brand">{incentivoSel.headline}</p>
            </div>
          </div>
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {incentivoSel.beneficios.map((b) => (
              <div key={b.titulo} className="rounded-xl border border-stone-200/50 bg-white/80 p-3.5 dark:border-stone-700/50 dark:bg-stone-900/50">
          ...
          {/* Preços dos serviços profissionais na região (todas as camadas à vista) */}
          <div className="mt-4">
            <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.15em] text-stone-400">Serviços profissionais na região</p>
            <div className="grid gap-2 sm:grid-cols-3">
              {PROFISSOES_LISTA.map((p) => {
                const b = bandaProfissao(p.id, incentivoSel.id);
                return (
                  <button
                    key={p.id}
                    type="button"
                    onClick={() => setFiltro(p.id)}
                    title={`Ver a camada ${p.nome}`}
                    className="group rounded-xl border border-stone-200/60 bg-white/80 p-3 text-left transition-all hover:border-brand/40 hover:shadow-card dark:border-stone-700/60 dark:bg-stone-900/50"
                  >
                    <p className="text-[11px] font-semibold text-stone-500 group-hover:text-brand-dark dark:text-stone-400 dark:group-hover:text-brand">
                      {p.singular}
                    </p>
                    <p className="mt-0.5 text-sm font-bold tabular-nums text-stone-800 dark:text-stone-100">
                      {eur0(b.min)}–{eur0(b.max)}
                      <span className="text-[10px] font-medium text-stone-400">{p.unidade}</span>
                    </p>
                    <p className="text-[10px] text-stone-400">{p.indicador}</p>
                  </button>
                );
              })}
            </div>
      ...
      <div>
        ...
        <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {regioesOrdenadas.map((id) => {
            const r = REGIOES_PRECO.find((x) => x.id === id);
            if (!r) return null;
            const ativa = id === selecionada;
            const inc = filtro === "beneficios" ? REGIOES_INCENTIVO.find((x) => x.id === id) : null;
            const banda = filtro !== "beneficios" ? bandaProfissao(filtro, id) : null;
            return (
              <li key={`${filtro}-${id}`}>
                <button
                  type="button"
                  aria-pressed={ativa}
                  onClick={() => escolherRegiao(id)}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-[11px] text-stone-400">{inc ? inc.headline : r.referencia}</span>
                  </span>
                  <span
                    className={`flex-shrink-0 rounded-lg px-2 py-0.5 text-[11px] font-bold tabular-nums ${
                      ativa
                        ? "bg-brand/10 text-brand-dark dark:bg-brand/20 dark:text-brand"
                        : "bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-300"
                    }`}
                  >
                    {inc ? inc.selo : banda ? `${eur0(banda.min)}–${eur0(banda.max)}` : ""}
                  </span>
                </button>
              </li>
            );
      ...
      {/* ── Serviços & atos frequentes (camadas de profissões) ── */}
      {profissao && (
        <div className="rounded-2xl border border-stone-100 bg-stone-50 p-4 dark:border-stone-800 dark:bg-stone-900/50 sm:p-5">
          <div className="mb-3 flex items-center gap-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand/10 text-brand dark:bg-brand/15">
              <def.Icon size={14} />
            </span>
            <p className="text-xs font-bold uppercase tracking-[0.15em] text-stone-500 dark:text-stone-400">
              Serviços frequentes, com preços
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            {profissao.servicos.map((s) => (
              <div key={s.nome} className="rounded-xl border border-stone-200/50 bg-white p-3.5 dark:border-stone-700/50 dark:bg-stone-800/60">
                <div className="flex items-start justify-between gap-3">
                  <p className="min-w-0 text-xs font-semibold text-stone-800 dark:text-stone-100">{s.nome}</p>
                  <span className="flex-shrink-0 rounded-lg bg-brand/8 px-2 py-0.5 text-[11px] font-bold tabular-nums text-brand-dark dark:bg-brand/15 dark:text-brand">
                    {eur0(s.min)}–{eur0(s.max)}
                    {s.unidade}
                  </span>
                </div>
                <p className="mt-1 text-[11px] leading-relaxed text-stone-500 dark:text-stone-400">{s.detalhe}</p>
                <p className="mt-1.5 text-[10px] font-medium uppercase tracking-wide text-stone-400">
                  {s.tabela ? "Valor de tabela / balcão público" : "Estimativa de mercado"}
                </p>
              </div>
            ))}
          </div>
      ...
      {/* ── Incentivos nacionais (camada de benefícios) ── */}
      {filtro === "beneficios" && (
        <div className="rounded-2xl border border-stone-100 bg-stone-50 p-4 dark:border-stone-800 dark:bg-stone-900/50 sm:p-5">
          <div className="mb-3 flex items-center gap-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand/10 text-brand dark:bg-brand/15">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {INCENTIVOS_NACIONAIS.map((b) => (
              <div key={b.titulo} className="flex items-start gap-2.5 rounded-xl border border-stone-200/50 bg-white p-3.5 dark:border-stone-700/50 dark:bg-stone-800/60">
                <Check size={13} className="mt-0.5 flex-shrink-0 text-brand" />
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-stone-800 dark:text-stone-100">{b.titulo}</p>
      ...
      {/* ── Contratar bem (camadas de profissões) ── */}
      {profissao && (
        <div>
          <div className="mb-3 flex items-center gap-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand/10 text-brand dark:bg-brand/15">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.15em] text-stone-500 dark:text-stone-400">Contratar bem</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            {profissao.comoEscolher.map((texto, i) => (
              <div key={i} className="flex items-start gap-3 rounded-2xl border border-stone-100 bg-white p-4 dark:border-stone-800 dark:bg-stone-900/60">
                <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-stone-100 text-xs font-bold text-stone-500 dark:bg-stone-800 dark:text-stone-400">
                  {i + 1}
                </span>
                <p className="min-w-0 text-[11px] leading-relaxed text-stone-600 dark:text-stone-300">{texto}</p>
              </div>
            ))}
          </div>
          <a
            href={profissao.ordem.url}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 inline-flex max-w-full items-center gap-2 rounded-xl border border-stone-200 bg-white px-3.5 py-2.5 text-xs font-semibold text-stone-700 transition-all hover:border-brand/40 hover:text-brand-dark dark:border-stone-700 dark:bg-stone-800 dark:text-stone-200 dark:hover:text-brand"
          >
            <ShieldCheck size={14} className="flex-shrink-0 text-brand" />
            <span className="min-w-0 truncate">{profissao.ordem.nome}</span>
            <ExternalLink size={12} className="flex-shrink-0 text-stone-400" />
          </a>
          <p className="mt-1.5 text-[11px] text-stone-400">{profissao.ordem.nota}</p>
        </div>
      )}

      {/* ── Nota / metodologia + fontes ── */}
      {profissao ? (
        <div className="flex items-start gap-2.5 rounded-2xl border border-stone-100 bg-stone-50 px-4 py-3 dark:border-stone-800 dark:bg-stone-900/50">
          ...
          <p className="text-[11px] leading-relaxed text-stone-500 dark:text-stone-400">
            <strong className="text-stone-600 dark:text-stone-300">{profissao.nota}</strong>{" "}
            <InfoTip label="Como usamos isto">
              Os valores marcados como «tabela» vêm de tabelas reguladas ou balcões públicos e são iguais em todo o país.
              Os restantes são estimativas de mercado: centros urbanos tendem a praticar honorários mais altos, e a
              complexidade do caso pesa mais do que a região.
            </InfoTip>{" "}
            Última revisão: {VERIFICACOES[profissao.id]}.{" "}
            {profissao.fontes.map((f, i) => (
              <span key={f.url}>
                {i > 0 ? " · " : "Fontes: "}
                <a
                  href={f.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline decoration-stone-300 underline-offset-2 hover:text-brand"
                >
                  {f.label}
                </a>
              </span>
        ...
        <p className="text-[11px] leading-relaxed text-stone-400">
          ...
          A taxa de IRC de 12,5% aplica-se a concelhos classificados como território do interior (não a uma região
          inteira){" "}
      ...
      <style>{`@keyframes rc-mapa-in{from{transform:translateY(10px);opacity:0}to{transform:translateY(0);opacity:1}}@media (prefers-reduced-motion: reduce){@keyframes rc-mapa-in{from{opacity:0}to{opacity:1}}}`}</style>
