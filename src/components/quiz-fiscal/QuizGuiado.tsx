  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:py-10">
      ...
      <div className="mb-6 flex items-center justify-between text-sm">
        ...
        <span className="rounded-full bg-stone-100 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-stone-400 dark:bg-stone-800">
          Sem tempo limite
        </span>
      </div>
                  ...
                  {mostrarExplicacao && (estado === "correta" || estado === "errada") && (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="mt-1 text-stone-600 dark:text-stone-300">{opcao.porque}</p>
                      </div>
                    </m.div>
                  )}
                </AnimatePresence>
      ...
      <div className="mt-6 flex justify-end">
        {!respondida ? (
          <Button variant="primary" size="lg" disabled={selecionada === null} onClick={confirmarResposta}>
            Confirmar resposta
          </Button>
        ) : (
        ...
        {sessao.map((_, i) => (
          <span
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              i === indice ? "w-5 bg-brand" : i < indice ? "w-1.5 bg-brand/40" : "w-1.5 bg-stone-200 dark:bg-stone-700"
            }`}
          />
        ))}
      </div>
...
function opcaoClasses(estado: "default" | "selecionada" | "correta" | "errada" | "apagada" | "eliminada"): string {
  ...
  switch (estado) {
    ...
    case "selecionada":
      return `${base} border-brand bg-brand-light/60`;
    default:
