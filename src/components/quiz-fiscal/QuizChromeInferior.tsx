...
// Chrome inferior do Quiz Fiscal (telemóvel) — fixo em baixo.
//  · Barra de estatísticas (Acertos/Tempo/Pontos/Erros) por cima do header.
//  · Header: configurações (esq.) + logótipo→home (esq.) · nível (centro, abre
//    modal de nível+perfil) · menu (dir., abre histórico).
// Usa os dados reais de useQuizProgresso (nível, XP, streak, energia, sessões).
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex min-w-0 flex-col items-center leading-tight">
      <span className="flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wide text-quiz-parchment/60">
        {icon}{label}
      </span>
...
export default function QuizChromeInferior({ acertos, erros, pontos, tempo, onConfiguracoes }: Props) {
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 lg:hidden">
        ...
        <nav className="flex items-center justify-between gap-2 border-t border-stone-200 bg-white px-3 pb-[max(0.4rem,env(safe-area-inset-bottom))] pt-2 dark:border-stone-800 dark:bg-stone-900" aria-label="Navegação do quiz">
          {/* Esquerda: configurações + logótipo */}
          <div className="flex items-center gap-1">
            <button type="button" onClick={onConfiguracoes} aria-label="Configurações / sair" className="flex h-10 w-10 items-center justify-center rounded-lg text-stone-500 transition-colors hover:bg-stone-100 dark:text-stone-400 dark:hover:bg-stone-800">
              <Settings size={20} />
            </button>
      ...
      {/* ── Modal de nível + perfil (sobe) ── */}
      <AnimatePresence>
        {modal === "nivel" && (
          <ModalBase titulo="O teu nível" onClose={() => setModal(null)}>
            <div className="flex items-center gap-4">
              <span className="relative flex h-16 w-16 flex-shrink-0 items-center justify-center font-display text-2xl font-bold text-white" style={{ background: "#1D9E75", clipPath: HEX }}>
                {nivel.nivel}
              </span>
              <div className="min-w-0 flex-1">
                ...
                <span className="mt-2 block h-2 overflow-hidden rounded-full bg-stone-200 dark:bg-stone-700">
                  <span className="block h-full rounded-full bg-brand" style={{ width: `${Math.min(100, xpPct)}%` }} />
                </span>
                <p className="mt-1 text-[11px] text-stone-400">
                  {nivel.xpProximo != null ? `Faltam ${xpRestante.toLocaleString("pt-PT")} XP para o nível ${nivel.nivel + 1}` : "Nível máximo atingido"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-xl border border-stone-100 bg-stone-50 p-4 dark:border-stone-800 dark:bg-stone-800/40">
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <span className="flex h-11 w-11 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand text-base font-bold text-white">
                  ...
                  <Link href="/dashboard/conta" onClick={() => setModal(null)} className="flex h-9 items-center gap-1 rounded-lg bg-white px-3 text-xs font-semibold text-stone-700 shadow-card dark:bg-stone-900 dark:text-stone-200">
                    Conta <ChevronRight size={13} />
                  </Link>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs text-stone-500 dark:text-stone-400">Entra para guardares o teu nível e histórico na nuvem.</p>
                  <button type="button" onClick={() => { setModal(null); abrirModal("entrar"); }} className="flex h-9 flex-shrink-0 items-center rounded-lg bg-brand px-3.5 text-xs font-bold text-white shadow-glow">
                    Entrar
                  </button>
                </div>
              )}
              ...
              {NIVEIS.map((n) => {
                ...
                return (
                  <li key={n.nivel} className={`flex items-center gap-3 rounded-lg px-3 py-2 ${atual ? "bg-brand-light" : ""}`}>
                    <span className={`flex h-7 w-7 flex-shrink-0 items-center justify-center text-xs font-bold ${alcancado ? "text-white" : "text-stone-400"}`} style={{ background: alcancado ? "#1D9E75" : "transparent", border: alcancado ? "none" : "1.5px solid #d6d3d1", clipPath: HEX }}>
                      {n.nivel}
                    </span>
      ...
      {/* ── Menu (sobe) com histórico funcional ── */}
      <AnimatePresence>
        {modal === "menu" && (
          <ModalBase titulo="Menu" onClose={() => setModal(null)}>
            <div className="space-y-1.5">
              ...
              <Link href={user ? "/dashboard/conta" : "/quiz-fiscal"} onClick={(e) => { if (!user) { e.preventDefault(); setModal(null); abrirModal("entrar"); } else setModal(null); }} className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-stone-50 dark:hover:bg-stone-800">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-light text-brand"><User size={18} /></span>
                <span className="flex-1 text-sm font-semibold text-stone-800 dark:text-stone-100">{user ? "A minha conta" : "Entrar / criar conta"}</span>
                <ChevronRight size={14} className="text-stone-300" />
              </Link>
            </div>
            ...
            {prog.sessoes.length === 0 ? (
              ...
              <ul className="space-y-1.5">
                {prog.sessoes.map((s) => <SessaoRow key={s.id} s={s} />)}
              </ul>
            )}

            {onConfiguracoes && (
              <button type="button" onClick={() => { setModal(null); onConfiguracoes(); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-stone-200 px-4 py-3 text-sm font-semibold text-stone-600 dark:border-stone-700 dark:text-stone-300">
    ...
    <li className="flex items-center gap-3 rounded-xl border border-stone-100 bg-white px-3 py-2.5 dark:border-stone-800 dark:bg-stone-900">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-stone-800 dark:text-stone-100">
          {s.acertos}/{s.totalPerguntas} certas · <span className="capitalize">{s.modo}</span>
        </p>
...
/** Folha inferior reutilizável (sobe). */
function ModalBase({ titulo, onClose, children }: { titulo: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-[90] lg:hidden" role="dialog" aria-modal="true" aria-label={titulo}>
      ...
      <m.div
        initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 32, stiffness: 330 }}
