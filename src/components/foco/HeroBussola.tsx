...
const ComoFuncionaModal = dynamic(async () => {
  const [{ default: Modal }, { default: MotionProvider }] = await Promise.all([
    import("@/components/ComoFuncionaModal"),
    import("@/components/ui/motion/MotionProvider"),
  ]);
  return function ComoFuncionaComMovimento(
...
export default function HeroBussola({
  ...
  const palco = usePalco(ATOS_BUSSOLA, palcoRef, "palco-bussola");
  ...
  const [comoFuncionaMontado, setComoFuncionaMontado] = useState(false);
  const abrirComoFunciona = useCallback(() => {
    setComoFuncionaMontado(true);
      ...
      >
        ...
        <div className="mx-auto max-w-5xl">
          ...
          {comoFuncionaMontado ? (
            <ComoFuncionaModal
              aberto={comoFunciona}
              onFechar={() => setComoFunciona(false)}
            />
          ) : null}
        </div>
      </section>
