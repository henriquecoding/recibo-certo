...
export default function SubscricaoAutenticada({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (sessionId && !reconciled) {
          ...
          if (result.reembolsado) {
            ...
            // Pagamento ainda a confirmar: o id fica para a tentativa seguinte.
          } else if (response.ok) {
            reconciled = true;
            clearCheckoutParams();
          } else {
  ...
  const abrirCheckout = useCallback(async (modalidade: Modalidade = "mensal") => {
    ...
    return {
      erro: result.erro ?? "Não foi possível iniciar o pagamento.",
      esgotado: Boolean(result.esgotado),
    };
  }, []);
  ...
  return <SubscricaoCtx.Provider value={contextValue}>{children}</SubscricaoCtx.Provider>;
}
