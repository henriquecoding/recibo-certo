...
//
//  Vive à parte de `version.ts` por uma razão de peso — literalmente: são mais
//  de 100 KB de texto que, importados por um componente-cliente, iam parar ao
//  browser em TODAS as páginas do site, para serem mostrados no máximo uma vez
//  por versão. `NovidadesModal` passa a carregá-lo sob demanda (`import()`),
//  só quando o popup abre de facto.
//
//  ⚠️ REGRA: a cada merge para `main`, sobe `APP_VERSION` (em `version.ts`) e
//  acrescenta uma entrada NO TOPO deste array. `assertChangelogIntegrity()`
//  corre ao importar este módulo e FALHA o build se as duas coisas
//  divergirem; o `layout.tsx` da raiz importa-o precisamente para essa
//  verificação continuar a acontecer em cada build, apesar de o conteúdo já
//  não seguir para o bundle do cliente.
// ═══════════════════════════════════════════════════════════════════════
...
export const CHANGELOG: EntradaChangelog[] = [
  ...
  },
  {
    ...
    itens: [
      ...
      "A secção «Casa e património» está escrita por inteiro — treze guias, do IMI à herança, passando pela venda com mais-valias, pelo arrendamento, pelo alojamento local e pela decisão de comprar em nome próprio ou pela empresa. Foram redigidos com o articulado aberto ao lado, artigo a artigo.",
      "Começou «Investir e poupar»: o Anexo J e as contas no estrangeiro, as corretoras que não comunicam nada à Autoridade Tributária, a regra dos 365 dias na cripto e os juros de depósitos e certificados.",
      "Duas coisas que quase toda a gente perde: uma conta de corretora aberta lá fora tem de ser identificada mesmo com saldo zero e sem rendimentos nenhuns; e o reporte de perdas para os cinco anos seguintes só existe se optares pelo englobamento no ano em que perdeste — quem deixa correr a taxa especial por defeito no ano mau fica sem nada para abater no ano bom.",
      ...
      "As reduções de IRS por contratos de arrendamento longos são em PONTOS PERCENTUAIS, não em percentagem da taxa: dez pontos sobre 25 % dá 15 %, não 22,5 %. E o arrendamento não habitacional — loja, escritório, armazém — fica nos 28 %, não nos 25 %.",
      "Quem vende casa passou a ter uma segunda via para não pagar mais-valias: reinvestir em imóveis para arrendamento habitacional com renda dentro dos limites do Decreto-Lei n.º 97/2026. Já está em vigor.",
      "As taxas do imposto do selo não estão em artigo nenhum do Código — estão na Tabela Geral, que só existe no PDF que a Autoridade Tributária publica. Passou a ser fonte citada, para os 0,8 % da escritura e os 0,60 % do crédito deixarem de ser números sem nada por trás.",
      ...
      "Os impostos do património — IMI, IMT, AIMI, imposto do selo, rendas e alojamento local — passaram a viver no motor fiscal, com base legal, fonte e data de verificação em cada valor, como já acontecia com o IRS e a Segurança Social. Nenhuma taxa está escrita no texto de um guia: quando a lei mudar, muda num sítio só.",
      "Vários destes parâmetros estavam sustentados num guia de consultora. Onde a lei fixa o número, passa a ser a lei a sustentá-lo — foram lidos artigo a artigo no Portal das Finanças.",
  ...
  },
  {
    version: "2.5.0",
    data: "2026-07-29",
    titulo: "Simulador de IRS: o imposto de quem tem salário passa a ser calculado como deve",
    itens: [
      "O mínimo de existência passa a aplicar-se a salários e pensões, e não só a recibos verdes. Quem ganha por volta do salário mínimo via um valor de IRS a pagar que a lei não permite — agora vê zero (ou quase, consoante as faturas com NIF que tenha).",
      "A dedução do trabalho dependente deixa de ser um valor fixo: acima de cerca de 41 700 € de rendimento bruto, passa a contar o total dos descontos para a Segurança Social, que é maior. Podes também indicar quotizações sindicais e de ordens profissionais, que aumentam a dedução.",
      "A tributação conjunta deixou de estar disponível a quem não é casado nem unido de facto. Era possível escolhê-la por engano e o imposto aparecia mais baixo do que a lei permite.",
      "O IRS Jovem saiu do módulo de recibos verdes e passou para a etapa do agregado: aplica-se ao salário e aos recibos verdes da mesma pessoa, com um único limite anual partilhado entre os dois.",
      "Deduções à coleta mais certas: as despesas gerais familiares deixaram de gastar teto que não é delas, as pensões de alimentos passaram a contar para esse teto, o limite dos casais passa a ser calculado sobre metade do rendimento (como manda a lei) e sobe 5% por cada filho a partir do terceiro.",
      "A exclusão por deficiência passou a abranger salários e pensões, e não só os recibos verdes, com o limite próprio de cada uma dessas origens.",
      "Rendimentos do estrangeiro: cada entrada passa a ter categoria, para receber a mesma dedução que teria em Portugal. Já é possível declarar rendimentos que a convenção isenta cá, e o crédito de imposto pago lá fora deixou de ser calculado por excesso.",
      "Retenções: a do salário passa a ser estimada pelas tabelas oficiais de 2026 quando não a souberes de cor, e a dos dividendos e juros de origem nacional deixa de aparecer como imposto por pagar — os 28% já foram retidos na fonte.",
      "Na venda de casa com reinvestimento, podes indicar o empréstimo amortizado na venda — conta a favor da exclusão da mais-valia.",
      "A página do simulador ficou mais leve e mais rápida a responder, e o resultado passa a sugerir o que fazer a seguir.",
    ],
  },
  {
    version: "2.4.0",
