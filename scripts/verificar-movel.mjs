...
/**
 * A versão do consentimento vem da FONTE, nunca escrita à mão aqui.
 *
 * Esteve um `1` literal neste ficheiro enquanto a aplicação já ia no `2`.
 * `lerConsentimento()` rejeita um registo com versão diferente, portanto o
 * banner de cookies abria por cima de tudo — e o portão passou uma corrida
 * inteira a medir o MODAL: 1008 «falhas» que eram, quase todas, o texto e os
 * botões da folha de consentimento. Um portão que mede a coisa errada é pior
 * do que não ter portão, porque tem a autoridade de um número.
 */
...
/* O perfil deixou de existir como estado: cada foco é a sua própria rota.
   Fica um valor só para o ciclo continuar a ser um ciclo. */
const PERFIS = ["-"];

/* As cinco rotas de foco. A homepage deixou de ramificar por um perfil em
   `localStorage` e passou a ser CINCO PÁGINAS — medir só `/` mediria um
   quinto do produto. São exactamente os cinco lugares da barra inferior. */
const PAGINAS = [
  { nome: "descobrir", url: "/" },
  { nome: "preco", url: "/inicio/preco" },
  { nome: "recibos", url: "/inicio/recibos" },
  { nome: "salario", url: "/inicio/salario" },
  { nome: "empresa", url: "/inicio/empresa" },
];
    ...
    for (const vp of VIEWPORTS) {
      for (const tema of ["claro", "escuro"]) {
        for (const perfil of PERFIS) {
          ...
          await ctx.addInitScript(({ t, versao, p, consentVersao }) => {
            try {
          ...
          }, { t: tema === "escuro" ? "dark" : "light", versao: APP_VERSION, p: perfil, consentVersao: CONSENT_VERSION });
          ...
          for (const pg of PAGINAS) {
            ...
            const porMontar = await page.evaluate(() => {
              ...
              if (!palco) falta.push("nenhum [data-palco] no DOM");
              else if (palco.getBoundingClientRect().height < 200) {
                falta.push(`palco com ${Math.round(palco.getBoundingClientRect().height)}px de altura`);
              }
              if (/A carregar/i.test(document.body.textContent || "")) falta.push("ainda há esqueletos a carregar");
              // Um modal por cima da página faz o portão medir o modal. Já
              // aconteceu (ver a nota do CONSENT_VERSION), e o sintoma foi um
              // relatório com mil falhas que não eram da página.
              const modal = document.querySelector("[role='dialog'], [aria-modal='true']");
              if (modal && modal.getBoundingClientRect().height > 100) {
                falta.push(`há um modal aberto por cima da página («${(modal.textContent || "").trim().slice(0, 40)}…»)`);
              }
              if (document.body.innerText.trim().length < 400) falta.push("a página quase não tem texto");
              return falta;
            });
            if (porMontar.length) {
